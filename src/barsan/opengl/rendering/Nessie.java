...
public class Nessie extends Renderer {
	...
	private void renderSLVol(SpotLight l, boolean computeLight) {
		...
		/*  Math time!
		 *  ----------
		 *  l.getCosOuter is the cosine of the angle of the WHOLE cone tip. However,
		 *  we will only do the computations for half of that angle, to take
		 *  advantage of the right angle generated by the cone's height and its
		 *  base plane.
		 *  
		 *  This way we just use basic trigonometry and multiply one of the cathetus
		 *  with the tangent of its adjacent non-right angle to get the length of
		 *  the other cathetus. We then multiply that by two to get the actual
		 *  width of the cone volume's base.
		 */
		float w = (float)( 2.0f * h * (Math.tan( Math.acos(l.getCosOuter() / 2.0f ))));
